...
/*
* Recebe um inteiro e remove o registro, se existir, em nível lógico do arquivo.
* @param {int} input
*/
void removeRecord(int input) {
  FILE *f = fopen(OUTPUT_FILE, "rb+");
  ...
  if(f == NULL) {
    ...
    while((r.value != input) || (i <= header.possibleTotalRecords)) {
      index = (h1 + (i++ * h2)) % header.possibleTotalRecords;
      position = sizeof(fileHeader) + (sizeof(record)*index);
